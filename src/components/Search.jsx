...
const Search = ({data}) => {
  const {byCat, byName, byCast, dispatch} = useContext(SearchContext)
  const handleClose = () => {
    dispatch({type: 'CANCEL_SEARCH'})
    dispatch({type: 'UPDATE_BY_CAT', payload:[]})
    dispatch({type: 'UPDATE_BY_NAME', payload:[]})
    dispatch({type: 'UPDATE_BY_CAST', payload:[]})
  }
  return (
    <div className="">
      ...
      <h2 className="sm:text-[24px] text-[18px] font-goemetric font-[400]">Search Results by Movie Title</h2>
        <div>
      ...
      <div className="md:px-[40px] px-[20px] pt-[30px] text-text-color">
      <h2 className="sm:text-[24px] text-[18px] font-goemetric font-[400]">Search Results by Movie Category</h2>
        <div>
          {byCat.length === 0 && <p className="sm:text-[18px] text-[16px] text-center my-[20px]">No Match Found</p> }
      ...
      <h2 className="sm:text-[24px] text-[18px] font-goemetric font-[400]">Search Results by Movie Cast</h2>
        <div>
          {byCast.length === 0 && <p className="sm:text-[18px] text-[16px] text-center my-[20px]">No Match Found</p> }
