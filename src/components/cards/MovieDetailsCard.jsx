          ...
          <Dialog open={open} onClose={handleClose}>
            <DialogContent style={{padding:0}}>
              ...
              <div className='details w-full text-text-color sm:px-[18px] px-[8px] py-[33px] flex flex-col gap-[25px] '>
                ...
                <div className='z-10'>
                  <h2 className='uppercase sm:text-[24px] text-[16px] font-[700]'>{data.title.length > 60 ? `${data.title.slice(0, 10)}...` : data.title}</h2>
                  <p className='sm:text-[16px] text-[12px]'>{data.plot.length > 200 ? `${data.plot.slice(0, 200)}...` : data.plot}</p>
                </div>
